...
describe('Profile Management', () => {
  it('Should change the password and change it back', () => {
    ...
    cy.get('.shade-tabs-header-container')
    ...
    cy.contains('Confirm new password')
      .find('input')
      .type('wrongNewPasswordConfirmation')
    cy.get('button')
      .contains('Change password')
      .click()
  ...
  it('Should update the theme from the personal settings JSON', () => {
    ...
    login(cy, 'testuser@gmail.com', 'password')
    navigateFromUserMenu(cy, 'Profile')
    cy.get('shade-tabs div')
      .contains('Personal settings', { matchCase: false })
      .scrollIntoView()
      .click()
